...
mock_heatmap_module_data <- tibble::tibble(
  ...
  full_month_aggregation = c(TRUE, FALSE, FALSE),
  full_week_aggregation = c(FALSE, TRUE, FALSE),
  PULocation = c("Loc_A", "Loc_A", "Loc_B"),
  DOLocation = c("Loc_X", "Loc_Y", "Loc_Z"),
  total_number_trips = c(10, 20, 30)
)
...
test_that("mod_yellow_taxis_heatmap_server processes inputs and KPIs correctly", {
  ...
  testServer(
    ...
    expr = {
      ...
      #  clean data: 1 row, 30 trips
      expect_equal(nrow(heatmap_data_combined), 1)
      expect_equal(sum(heatmap_data_combined$trips), 30)
