fn_monthly_aggregation <- function(df, grouping_values = c()) 
{
  ...
  if (length(missing_required) > 0) {
    log_error(".... missing required columns in input data frame: ", paste0(missing_required, collapse = ", "))
    return(NULL)
  }
  ...
  if (length(missing_grouping) > 0) {
    log_error(".... grouping columns not found in data frame: ", paste0(missing_grouping, collapse = ", "))
    return(NULL)
  }
  ...
  if("trip_start_date" %in% names(tmp)) {
    ...
    if (!"trip_start_date" %in% names(df)) {
      log_error(".... column 'trip_start_date' is required in the original data frame for monthly fallback aggregation.")
      return(NULL)
    }
