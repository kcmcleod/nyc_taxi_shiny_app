fn_process_yellow_file <- function(fullFileName) {
  ...
  tDF_clean$trip_start_week <- floor_date(tDF_clean$tpep_pickup_datetime, unit = "weeks", week_start = 7)
  ...
  remove(t2, t3, t4)
  log_info(".... returning comnbined dataset: ", ncol(combined_summary_df), " x ", nrow(combined_summary_df))
